...
if (ncol(gencov) == 5) {
  mult <- FALSE
  gencov <- read.table(covfile, header = F)[, c(1,2,3,5)]
  colnames(gencov) <- c("contig", "start", "end", "alignments")
} else {
  mult <- TRUE
  gencov <- read.table(covfile, header = F)[, c(1,2,3,6,7)]
...
if (nrow(contigs) > 30){
  ...
  gencov <- filter(gencov, contig %in% contigs$contig[1:30])
   ...
   pltheight <- round(10 * (nrow(contigs) / 3), digits = 0)
}
...
```{r echo = FALSE, message = FALSE, warning = FALSE, fig.height=pltheight, out.width = "150%"}
if (mult) {
