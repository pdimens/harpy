...
```{r Individual Stats, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, out.width="100%"}
...
### plot
```{r Locus Stats, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, out.width="100%"}
.dpL <- grepl("^DP", dataL)
if(sum(.dpL) == 0){
...
if(sum(.dpL) == 0){
  ...
  DT::datatable(dp, rownames = F, filter = "top", extensions = 'Buttons', options = list(dom = 'Brtip', buttons = c('csv')))
}
```
