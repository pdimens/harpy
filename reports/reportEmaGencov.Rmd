...
all_depth_filt <- nonoutliers$Barcode + abs(nonoutliers$noBarcode)
...
filt_df <- data.frame(
  "contig"            = "global",
  "average_all"       = round(global_avg_filt, 2),
  "sdv_all"           = round(global_sd_filt, 2),
  "average_Barcode"   = round(bc_avg_filt, 2),
  "sdv_Barcode"       = round(bc_sd_filt, 2),
  "average_noBarcode" = round(nobc_avg_filt, 2),
  "sdv_noBarcode"     = round(nobc_sd_filt, 2)
  )
```

```{r pivot_table, echo = FALSE, message = FALSE, warning = FALSE}
# pivot longer for generalized plotting
Lgencov <- pivot_longer(gencov[,-6], -(1:3), names_to = "alignment", values_to = "depth")
```
...
distplot <- ggplot(Lgencov, aes(x = depth, fill = alignment)) +
  geom_histogram(binwidth = 10, linewidth = 2) +
  theme_minimal() +
...
### Averages
The table below shows the per-contig average depth and standard 
deviation per 10kbp interval, **including** intervals whose depth is flagged
an outlier in the data.
```{r echo = FALSE, message = FALSE, warning = FALSE}
...
DT::datatable(
  ...
  colnames = c('Contig', 'Average Barcode', 'StDev Barcode', 'Average noBarcode', 'StDev noBarcode'),
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
Lnonoutliers <- pivot_longer(nonoutliers, -(1:3), names_to = "alignment", values_to = "depth")


contig_avg_filt_non <- group_by(Lnonoutliers, contig, alignment) %>%
  summarize(average = round(mean(abs(depth)),2), sdv = round(sd(abs(depth)),2))

contig_avg_filt_non <- pivot_wider(data = contig_avg_filt_non, names_from = alignment,values_from = c(average,sdv))#[, c(1,3,6,2,5,4,7)]

contig_avg_filt_non <- rbind(filt_df, contig_avg_filt_non)
...
.l <-  nrow(.contigs)
```
```{r eval = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.align='center', out.width= "100%"}
