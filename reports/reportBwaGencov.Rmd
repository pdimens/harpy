...
to the global mean depth (`r round(global_avg_filt,0)`) after removing outliers.
These outlier regions are marked by the orange vertical lines and you may investigate
them further using the table above.
...
```{r echo = FALSE, message = FALSE, warning = FALSE, fig.height=pltheight, out.width = "100%"}
  ggplot(.gencov, aes(x = start, y = depth)) +
    geom_area(stat = "identity", fill = "#3f3f3f") +
    geom_vline(data = outliers, color = "#cb6843", aes(xintercept = start)) +
    theme_minimal() +
    facet_wrap(~contig, ncol = 2, scales = "free_x") +
    labs(x = "position (bp)", y = "depth") +
    scale_y_continuous(labels = scales::comma) +
    scale_x_continuous(labels = scales::comma, guide = guide_axis(n.dodge = 2)) +
    theme(
