...
circosplot <- function(data, contigdata, svtype, trackheight, poplist, trackcolor){
  circos.clear()
  col_text <- "grey40"
  ...
  for(i in 1:length(poplist)){
    ...
    circos.genomicTrack(
      ...
      circos.genomicRect(region, value, ybottom = 0, ytop = 1, col = col.ramp[i], border = col.ramp[i])
      }
    )
  }
  
  text(0, 0, svtype, cex = 1.5)
}
```
...
for (SV in c("INV", "DEL", "DUP", "BND")){
  circosplot(sv, fa.sizes, SV, popheight, populations, "#FFC107")
}
lgd <- Legend(
    at = populations,
    type = "grid",
    legend_gp = gpar(fill = col.ramp),
