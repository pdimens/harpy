...
infiles <- "/home/pdimens/ZS102.bedpe"
...
cleanbedpe <- function(x){
  ...
  popname <- data.frame("Population" = rep(gsub(".bedpe", "", basename(x)), nrow(y)))
  y <- cbind(popname, y)
  names(y)[2] <- "Contig"
...
infiles
sv <- do.call(rbind, lapply(infiles, function(x) cleanbedpe(x))
#.sv <- sv
...
circosplot <- function(data, contigdata, svtype, trackheight, poplist, colormap, draw.leg){
  circos.clear()
  col_text <- "grey40"
...
for (SV in c("inversion", "deletion", "duplication")){
  ...
  circosplot(sv, fa.sizes, SV, popheight, populations, col.ramp, lgnd)
}
...
color.palette <- c("deletion"="#5a8c84", "duplcation"="#ffd75f", "inversion"="#4a9fea")
l <- htmltools::tagList()
...
for (i in 1:nrow(fa.sizes)) {
  ...
  plt <- sv.filt %>%
    ...
    scale_y_continuous(
      ...
      labels = c("Inversion", "Duplication", "Deletion"),
      limits = c(0,4)
    ) +
