...
This tab (`General Stats`) shows overview information. Clicking `Per-Contig Stats`
...
To the right is a circular plot to visualize the distribution of SV across (up to) 30 of the largest contigs. This should help you assess
the presence/absence of expected variants. The legend on below shows which
...
frameheight = 1500 * nrow(fa.sizes)
```
...
plt <- sv %>%
    ...
    facet_grid(rows = vars(contig), scales = "free_x") +
    theme_light() +
    theme(
  ...
  ggplotly(plt, height = 950 * length(nrow(fa.sizes)), tooltip = "text")
