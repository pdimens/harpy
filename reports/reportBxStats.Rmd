...
nBX <- group_by(valids, contig) %>% 
  summarize(nBX = length(bx))

avgBX <- round(mean(nBX$nBX), digits = 2)

totuniqBX <- length(unique(valids$bx))

tots <- tb %>% 
    group_by(valid) %>%
    summarize(total = length(bx)) %>% 
    as.data.frame()
...
pretty sure there is a table that belongs here
...
- `contig`: name of the contig the barcode occurs on 
- `bx`:  the barcode 
...
as unique molecules when their distance is greater than `r mdist` base pairs along a single contig.
The "split" molecules are identified by a numerical suffix in their name, such
as the `.1` in `A01C22B96D11.1`. These suffixes were added only for reporting purposes and 
the barcodes in your alignment file **were not** altered. If aligned with BWA, Harpy added
a corresponding `MI:i:` (Molecular Identifier) tag that reflects these splits given
the molecule distance threshold you specified. If aligned with EMA, the EMA software
itself determines the splits and assigns the `MI:i` tag without user specification.
