...
class VCF():
    ...
    def find_biallelic_contigs(self):
        ...
        """
        self.print.log("Finding contigs in VCF with at least 5 biallelic SNPs")
    ...
    def check_phase(self):
        ...
        self.print.log("Checking if VCF file is phased ([green]PS[/] or [green]HP[/] tags)")
        with pysam.VariantFile(self.file) as _vcf:
    ...
    def match_samples(self, bamlist: list[str], prioritize_vcf: bool) -> None:
        ...
        self.print.log("Validating all samples are present between VCF and input alignments")
        with pysam.VariantFile(self.file) as _vcf:
    ...
    def match_contigs(self, contigs: list[str]):
        ...
        self.print.log("Validating input contigs against those in the input VCF")
        with pysam.VariantFile(self.file) as _vcf:
    ...
    def validate_region(self, region: str) -> tuple[str,int,int,int]:
        ...
        """
        self.print.log("Validating input regions to those in the input VCF")
        startpos = 0
        endpos = 0
        buffer = 0
