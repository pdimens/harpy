...
# Coverage Stats
```{r reset_env}
# clear out all the objects in memory to reduce RAM usage
rm(list = ls())
```
```{r imports}
...
highchart() |>
  ...
  hc_xAxis(ceiling = mol_q99, title = list(text = "depth")) |>
...
is the number of _molecules_ spanning that interval, where molecules are inferred from the linked-read information with a distance threshold of
`r snakemake@params$mol_dist`.  
