...
tb <- merge(x=tb,y=tbmol, 
      by.x=c("contig","position"), 
      by.y=c("contig","position"),
      all.x=TRUE
)
...
### filt-avg
```{r valuebox_lessQ99}
valueBox(round(global_avg_filt,2), caption = '↓Q99 avg', color = "info")
```

### filt-sd
```{r valuebox_lessQ99_stdv}
valueBox(round(global_sd_filt,2), caption = '↓Q99 stdev', color = "info")
```
...
hchart(hs, "areaspline", hcaes(x = val, y = freq), color = "#9393d2", name = "% of intervals", marker = list(enabled = FALSE)) |>
