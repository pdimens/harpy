...
Below are three visualizations showing the occurrence of invalid haplotag barcodes.
...
the proportion of invalid haplotag barcode segments (A, C, B, or D) per individual.
...
```{r}
figheight <- 0.6 + (0.2 * nrow(invalids))
```

## validity plots {data-height=pltheight}
### count of valid barcodes {.no-title}
```{r valid_plot, fig.height=figheight, out.width = '100%'}
tb<-plotdata %>%
...
```
### proportional valid barcodes {.no-title}
```{r valid_proportion_plot, fig.height=figheight, out.width = '100%'}
...
### which haplotag barcode segments are invalid {.no-title}
```{r invalid_segment_plot, fig.height=figheight, out.width = '100%'}
highchart() |>
