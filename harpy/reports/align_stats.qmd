...
list(
  color = "light",
  value = scales::label_comma(length(unique(tb$contig)))
)
```
...
list(
  color = "#d4d4d4",
  value = scales::label_comma(totuniqBX)
)
```
...
list(
  color = "#d4d4d4",
  value = scales::label_comma(length(unique(valids$molecule)))
)
```
...
if (VALID_PRESENT){
  boxval <- scales::label_comma(avgBX)
} else {
  boxval <- "NA"
}
list(
