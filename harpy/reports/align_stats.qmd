...
using("dplyr","highcharter","DT","interacCircos")
```
...
if(!VALID_PRESENT){
  ...
  column_description <- c(
    ...
    "number of invalid linked-read barcodes, given by a segment with 00",
...
column_description <- c(
  ...
  "average depth of inferred molecules, which includes gaps between linked-reads",
...
```{r circleplot, fig.align='center', out.width= "80%", out.height="900px", eval = FALSE}
...
```{r circosplot, out.height="900px"}
#| title: Depth and Coverage Across the Genome (refresh browser if not loading)

aln_depth <- CircosHistogram(
  'DEPTH',
  data = data.frame(chr = tb$contig, start = tb$position, end = tb$position_end, name = "sequence", value = pmin(tb$depth, q99)),
  fillColor= "#757575",
  minRadius = 0.2 * 385,
  maxRadius = 0.55 * 385,
...
mol_depth <- CircosHistogram(
  'MOLDEPTH',
  data = data.frame(chr = tb$contig, start = tb$position, end = tb$position_end, name = "molecule", value = pmin(tb$mol_depth, mol_q99)),
  fillColor= "#9c3b94",
  minRadius = 0.64 * 385,
  maxRadius = 0.96 * 385,
  animationDisplay = FALSE
...
Circos(
  moduleList = aln_depth + mol_depth,
  genome = genomeChr,
  innerRadius = 385,
  outerRadius = 400,
  height = 800,
  width = "100%",
  HISTOGRAMMouseOverColor = "#1cff42",
  HISTOGRAMMouseOverDisplay = TRUE,
  HISTOGRAMMouseOutDisplay = TRUE,
  HISTOGRAMMouseOutColor = NULL,
