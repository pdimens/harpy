...
#| title: "% Singletons"
if (VALID_PRESENT){
  boxval <- round(sum(valids$reads < 2)/nrow(valids), 2)
} else {
  boxval <- "NA"
}
list(
...
Here you can see the frequency distribution of molecule lengths 
...
## breadth
:::{.card title="Inferred Molecule Read Coverage"}
This charts shows the frequency distribution of molecule coverage, meaning
the percent of each inferred molecule that has sequences aligned to it. In other
words, "_how much of each unique long molecule is actually sequenced/aligned?_"
