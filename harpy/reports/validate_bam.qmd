...
list(
  icon = "files",
  color = "light",
  value = scales::label_comma(nrow(data))
)
```
...
list(
  ...
  value = scales::label_comma(attention)
)
```
...
list(
  ...
  value = scales::label_comma(noBXtag)
)
```
...
list(
  icon = ifelse(noMItag > 0, "exclamation-triangle", "check2-square"),
  color = ifelse(noMItag > 0, "warning", "success"),
  value = scales::label_comma(noMItag)
)
```
...
list(
  ...
  value = scales::label_comma(bxnotlast)
)
```
