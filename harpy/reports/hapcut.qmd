...
list(
  color = "light",
  value = scales::comma(overall$total_snps[1])
)
```
...
list(
  color = "#dfdfdf",
  value = paste0(scales::comma(overall$max_blocksize[1]), "bp")
)
```
...
list(
  color = "#dfdfdf",
  value = scales::comma(overall$mean_snps[1])
)
```
...
list(
  color = "#dfdfdf",
  value = scales::comma(overall$median_snps[1])
)
```
```{r}
#| content: valuebox
...
list(
  color = "info",
  value = paste0(scales::comma(overall$N50[1]), "bp")
)
```
...
list(
  color = "info",
  value = paste0(scales::comma(overall$N75[1]), "bp")
)
```
...
list(
  color = "info",
  value = paste0(scales::comma(overall$N90[1]), "bp")
)
```
...
::: {.card title="Stats Per Contig" expandable="true"}
