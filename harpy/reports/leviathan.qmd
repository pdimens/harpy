...
using("dplyr","DT","interacCircos", "htmltools")
```
...
# Per-Contig Plot
```{r filter_contigs}
fa.sizes <- read.table(fai, header = F)[,1:2] %>% arrange(desc(2))
...
# Function to decide text color based on background luminence
...
```{r circosplot_background}
#| title: "Putative Structural Variants (refresh browser if not appearing)"

# breakends outer rim
tracks <- CircosBackground(
  "BG_INVERSIONS",
  minRadius = 0.75 * 385,
  maxRadius = 0.89 * 385,
  fillColors = "#f3f3f3",
...
tracks <- tracks + CircosBackground(
  "BG_BREAKENDS",
  minRadius = 0.9 * 385,
  maxRadius = 0.98 * 385,
  fillColors = "#f3f3f3",
...
inv <- sv[sv$type == "INV",]
if(nrow(inv) > 0){
  tracks <- tracks + CircosArc(
    'INVERSIONS',
    data = data.frame(chr = inv$contig, start = inv$position_start, end = inv$position_end, value = 0, des = "inversion", color = color.palette["inversion"]),
    innerRadius = 0.76 * 385,
    outerRadius = 0.88 * 385,
    )
}
...
del <- sv[sv$type == "DEL",]
if(nrow(del) > 0){
  tracks <- tracks + CircosSnp(
    'DELETIONS',
    minRadius = 0.6 * 385,
    maxRadius = 0.74 * 385,
    data = data.frame(chr = del$contig, pos = del$position_start, value = del$position_end - del$position_start ,des = "deletion"),
    fillColor = color.palette["deletion"],
  )
}
...
dup <- sv[sv$type == "DUP",]
if(nrow(dup) > 0){
  tracks <- tracks + CircosCnv(
    'DUPLICATIONS',
    data = data.frame(chr = dup$contig, start = dup$position_start, end = dup$position_end, value = "duplication"),
    minRadius = 0.45 * 385,
    maxRadius = 0.59 * 385,
    color = color.palette["duplication"],
    strokeColor = color.palette["duplication"]
  )
}
...
bnd <- sv[sv$type == "BND",]
if(nrow(bnd) > 0){
  tracks <- tracks + CircosScatter(
    'BREAKENDS',
    radius = .95 * 385,
    data = data.frame(chr = bnd$contig, start = bnd$position_start, start = bnd$position_end, name = "breakend"),
    innerCirclecolor = color.palette["breakend"],
    outerCirclecolor = color.palette["breakend"],
  )
}
```

```{r circos_plot, out.height="900px"}
Circos(
  moduleList = tracks,
  genome = genomeChr,
  innerRadius = 385,
