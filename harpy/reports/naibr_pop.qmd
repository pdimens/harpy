...
```{r read_inputs}
infiles <- list.files(path = params$bedpedir, pattern = "*bedpe", full.names = T)
fai <- params$faidx
#fai <- "~/testshad.fai"
#infiles <- params$bedpedir
```
...
list(
  color = "light",
  value = scales::comma(length(levels(sv$population)))
)
```
```{r}
#| content: valuebox
...
list(
  color = "#dfdfdf",
  value =scales::comma(ndel, accuracy = 0.1), 
)
```
...
list(
  color = "#dfdfdf",
  value= scales::comma(ndup, accuracy = 0.1)
)
```
...
list(
  color = "#dfdfdf",
  value = scales::comma(ninv, accuracy = 0.1)
)
```
...
##
::: {.card title="All Non-Chimeric Variants"}
...
This table shows any structural variants whose breakpoints span multiple contigs and may require further assessment. These variants omitted from the plots.
...
Each internal (grey) ring is detected inversions on that contig for a different population. Very small variants may be difficult to see or will not appear on the plot.
