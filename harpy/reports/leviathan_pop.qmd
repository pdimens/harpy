...
```{r read_inputs}
statsfiles <- list.files(path = params$statsdir, pattern = "*sv.stats", full.names = T)
faidx <- params$faidx
...
list(
  color = "#dfdfdf",
  value = scales::comma(ndel, accuracy = 0.01)
)
```
```{r}
#| content: valuebox
...
list(
  color = "#dfdfdf",
  value = scales::comma(ndup, accuracy = 0.01)
)
```
```{r}
#| content: valuebox
...
list(
  color = "#dfdfdf",
  value = scales::comma(ninv, accuracy = 0.01)
)
```
...
may be difficult to see or will not appear on the plot. If for some reason the plot isn't appearing, try refreshing
your browser.
...
small variants may be difficult to see or will not appear on the plot. If for some reason the plot isn't appearing, try refreshing
your browser.
