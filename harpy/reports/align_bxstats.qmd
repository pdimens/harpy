...
list(
  color = "light",
  value = scales::label_comma(round(mean(aggregate_df$N50[aggregate_df$N50 != 0]), 0))
)
```
...
list(
  color = "light",
  value = scales::label_comma(round(mean(aggregate_df$N75[aggregate_df$N75 != 0]), 0))
)
```
...
list(
  color = "light",
  value = scales::label_comma(round(mean(aggregate_df$N90[aggregate_df$N90 != 0]), 0))
)
```
...
hchart(aggregate_df, "scatter", hcaes(x = totalreads, y = round(nonsingletons / (nonsingletons + singletons), 2), color = averagereadspermol, name = sample, molper = averagereadspermol), name = "Linked-Read Performance", animation = F) |>
  ...
  hc_yAxis(max = 1, title = list(text = "proportion of reads with linked-read information")) |>
  hc_tooltip(
    crosshairs = FALSE,
