...
```{r setup_df, results = F}
indir <- params$indir
#indir <- .files("test", "bxstats.gz", full.names = TRUE)
...
- `molecules` the unique DNA molecules as inferred from linked-read barcodes
  - `singletons` molecules composed of one single-end or paired-end sequence, effectively not a linked read
  - `nonsingletons` molecules composed of more than two single-end or paired-end sequences
- `barcodes` are the linked-read barcodes associated with DNA sequences and are synonymous with `bx`
- `valid` a proper haplotag barcode (e.g. `A01C34B92D51`)
- `invalid` an invalidated haplotag barcode, where there is a `00` in any of the `ACBD` positions (e.g. `A21C00B32D57`)
- `NX` the N-statistics (explained in more detail below)
...
DT::datatable(
  ...
  fillContainer = T,
  colnames = c("sample", "alignments", "unique molecules", "singletons", "nonsingletons", "unique barcodes", "molecule:bx ratio", "valid bx alignments", "invalid bx alignments", "% valid bx", "avg reads/molecule", "SEM reads/molecule", "avg molecule coverage %","SEM molecule coverage %", "N50", "N75","N90")
)
```
...
## Per-sample plots
### percent valid
```{r perc_valid_per_sample, out.width="100%"}
...
```{r perc_nonsingleton, out.width="100%"}
hchart(aggregate_df, "xrange", animation = F, groupPadding = 0.0001,
...
### reads per molecule
```{r reads_per_mol_sample, out.width="100%"}
