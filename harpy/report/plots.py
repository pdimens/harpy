...
def SafeRender(data):
    '''
    Given a Polars DataFrame (or list), will temporarily set the Altair
    render engine to SVG to avoid the 5000 max row cutoff.
    '''
...
def piechart(df: pl.DataFrame, title: str = "", goodval: str | None = None, lgtitle: bool = False, lgpos: str = "bottom"):
    total = df["count"].sum()
    _names = df.columns
    _n = set(df[_names[0]].to_list())

    _color = alt.Color(
        f"{_names[0]}:N",
        title=_names[0].title(),
        legend=(
            alt.Legend(orient=lgpos, title=None)
            if not lgtitle
            else alt.Legend(orient=lgpos)
        ),
    )
...
def convolutionbar(df: pl.DataFrame, title: str = ""):
    total = df["count"].sum()
    _names = df.columns
...
def _makepanel(df: pl.DataFrame, metric, title=None):
...
def depthplot(df: pl.DataFrame, title):
    alt.data_transformers.disable_max_rows()
    ...
    if coverage and molcov:
        res = alt.vconcat(
            _makepanel(
                df,
                "Read Depth",
                title=f"{title} (Read Depth)",
            ),
            _makepanel(
                df,
                "Molecule Depth",
                title=f"{title} (Molecule Depth)",
            ),
        ).resolve_scale(
            x="independent",
            y="independent",
...
def sv_by_chromosome(
    variants: pl.DataFrame,
    title: str = "",
):
    '''
    Return an Altair chart of SVs and their positions in a chromosome.
    This includes a dropdown selection to display a specific chromosome.
    '''
    _sv = [
        "Inversion",
        "Deletion",
    ...
    dynamic_title = alt.Title(
        alt.expr(
            f'"Structural Variants on " + {selection.name}.Contig'
        ),
        subtitle="Variants should be considered putative",
    )

    return (
        alt.Chart(variants)
        .transform_calculate(
            var_length="datum.End - datum.Start"
        )
...
def depth_by_chromosome(records: pl.DataFrame, title: str = ""):
    ...
    length_param = alt.param(
        expr='max(pluck(data("data_0"), "position_end"))'
    )
