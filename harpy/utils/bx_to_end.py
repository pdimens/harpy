...
def bx_to_end(infile):
    ...
    if infile.lower().endswith(".bam") or infile.lower().endswith(".sam"):
        ...
        sys.stderr.write(f"Filetype not recognized as one of BAM or FASTQ for file {infile}\n")
        sys.exit(1)

    if is_fastq:
        with (
            FastxFile(infile, persist=False) as fq_in,
        ...
        try:
            bam_in = AlignmentFile(infile, "rb", require_index=False)
        except (OSError, ValueError):
            try:
                bam_in = AlignmentFile(infile, "r", require_index=False)
            except (OSError, ValueError) as e:
                print(f"Could not process {infile} as a SAM/BAM file. See the error from pysam: {e}")
                sys.exit(1)
        with AlignmentFile(sys.stdout.buffer, "wb", template = bam_in) as bam_out:
