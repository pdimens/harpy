...
from .printing import print_error, print_notice, print_solution, print_solution_offenders
...
def check_impute_params(parameters: str) -> dict:
    ...
    with open(parameters, "r", encoding="utf-8") as paramfile:
        ...
        if sorted(headersplt) != correct_header:
            ...
            print_error(
                "incorrect columns",
                f"Parameter file [bold]{parameters}[/] has incorrect column names" + "".join(errtext) + "\nValid names are: [green bold]" + " ".join(colnames) + "[/]",
                False
            )
        ...
        if len(badrows) > 0:
            ...
            print_error("invalid rows", f"Parameter file [blue]{parameters}[/] is formatted incorrectly. Not all rows have the expected {n_cols} columns.", False)
            print_solution_offenders(
        ...
        for k,v in data.items():
            ...
            row += 1
        if row_error:
            print_error("invalid parameter values", f"Parameter file [bold]{parameters}[/] is formatted incorrectly. Some rows have incorrect values for one or more parameters.", False)
            print_solution_offenders(
...
def validate_bam_RG(bamlist: str, threads: int, quiet: int) -> None:
    ...
    if len(culpritfiles) > 0:
        ...
        for i,j in zip(culpritIDs,culpritfiles):
            _out.append(f"{i}\t{j}")
        print_error("sample ID mismatch",f"There are [bold]{len(culpritfiles)}[/] alignment files whose RG tags do not match their filenames.", False)
        print_solution_offenders(
...
def validate_popfile(infile: str) -> None:
    ...
    with open(infile, "r", encoding="utf-8") as f:
        ...
        if invalids:
            print_error(
                "invalid format",
                f"There are [bold]{len(invalids)}[/] rows in [bold]{infile}[/] without a space/tab delimiter or don't have two entries for sample[dim]<tab>[/dim]population. Terminating Harpy to avoid downstream errors.",
                False
            )
            print_solution_offenders(
...
def validate_popsamples(infiles: list[str], popfile: str, quiet: int) -> None:
    ...
    if len(missing_samples) > 0:
        print_error(
            "mismatched inputs",
            f"There are [bold]{len(missing_samples)}[/] samples included in [blue]{popfile}[/] that weren\'t found in in the inputs. Terminating Harpy to avoid downstream errors.",
            False
        )
        print_solution_offenders(
...
def validate_demuxschema(infile:str) -> None | int:
    ...
    if len(code_letters) > 1:
        print_error("invalid schema", f"Schema file [blue]{os.path.basename(infile)}[/] has sample IDs occurring  in different barcode segments.", False)
        print_solution_offenders(
        ...
        with open(regioninput, "r", encoding="utf-8") as fin:
            for idx, line in enumerate(fin, 1):
                ...
                if row[0] not in contigs:
                    print_error(
                        "missing contig",
                        f"The contig listed at row {idx} ([bold yellow]{row[0]}[/]) is not present in ([blue]{os.path.basename(genome)}[/]). This is the first row triggering this error, but it may not be the only one.",
                        False
                    )
                    print_solution(
...
def check_fasta(genofile: str) -> str:
            ...
            if line.startswith(">"):
                ...
                if last_header:
                    print_error(
                        ...
                        f"All contig names must be followed by at least one line of nucleotide sequences, but two consecutive lines of contig names were detected. This issue was identified at line [bold]{line_num}[/] in [blue]{genofile}[/], but there may be others further in the file.",
                        False
                    )
                ...
                if len(line.rstrip()) == 1:
                    print_error(
                        "unnamed contigs",
                        f"All contigs must have an alphanumeric name, but a contig was detected without a name. This issue was identified at line [bold]{line_num}[/] in [blue]{genofile}[/], but there may be others further in the file.",
                        False
                    )
                    print_solution("See the FASTA file spec and try again after making the appropriate changes: https://www.ncbi.nlm.nih.gov/genbank/fastaformat/")
                if line.startswith("> "):
                    print_error(
                        ...
                        f"All contig names must be named [green bold]>contig_name[/], without a space, but a contig was detected with a space between the [green bold]>[/] and contig_name. This issue was identified at line [bold]{line_num}[/] in [blue]{genofile}[/], but there may be others further in the file.",
                        False
                    )
                    print_solution("See the FASTA file spec and try again after making the appropriate changes: https://www.ncbi.nlm.nih.gov/genbank/fastaformat/")
            elif line == "\n":
                print_error(
                    "empty lines",
                    f"Empty lines are not permitted in FASTA files, but one was detected at line [bold]{line_num}[/] in [blue]{genofile}[/]. The scan ended at this error, but there may be others further in the file.",
                    False
                )
                print_solution("See the FASTA file spec and try again after making the appropriate changes: https://www.ncbi.nlm.nih.gov/genbank/fastaformat/")
            else:
    ...
    if seq_id == 0:
        print_error("contig names absent", f"No contig names detected in [blue]{genofile}[/].", False)
    ...
    if seq == 0:
        print_error("sequences absent", f"No sequences detected in [blue]{genofile}[/].", False)
        print_solution(solutiontext + "\nSee the FASTA file spec and try again after making the appropriate changes: https://www.ncbi.nlm.nih.gov/genbank/fastaformat/")
...
def fasta_contig_match(contigs: str, fasta: str) -> None:
    ...
    if bad_names:
        ...
        print_error("contigs absent", f"Some of the provided contigs were not found in [blue]{shortname}[/]. This will definitely cause plotting errors in the workflow.", False)
        print_solution_offenders("Check that your contig names are correct, including uppercase and lowercase.", f"Contigs absent in {shortname}:")
    ...
    def validate(fastq):
        ...
        with safe_read(fastq) as fq:
            ...
            if not BX and not BC:
                print_error(
                    "no barcodes found",
                    f"No [green bold]BC:Z[/] or [green bold]BX:Z[/] tags were detected in read headers for [blue]{os.path.basename(fastq)}[/]. Athena requires the linked-read barcode to be present as either [green bold]BC:Z[/] or [/]BX:Z[/] tags.",
                    False
                )
