...
@click.option('-d', '--debug', is_flag = True, default = False, help = 'Log process information to stderr and store intermediate html files')
@click.help_option('--help', hidden = True)
@click.argument('notebooks', required=True, type=IPYNBfile(), nargs=-1)
def static(notebooks, debug):
    """
    Convert ipynb reports to standalone HTML files

    Uses `jupyter nbconvert` and Monolith to convert .ipynb files
    generated by Harpy workflows into their own HTML files that do
    not require binding into a single website or a live-server to be viewed.
    The generated HTML will contain the intended content of the reports, but
    will lack the nicer features (and some of the formatting) of a proper MyST-MD website. Generated HTML
    files are built in the same directories as their .ipynb files and will be
    named identically, save for the `.html` file extension. Assumes the notebooks
    are already executed; outputs are used as-is.
    """
    check_tool(
        "jupyter",
    ...
    check_tool(
        "monolith",
        "It is not (yet) bundled with Harpy and can be installed with "
    ...
    rs = ReportStatic("", quiet = not debug, intermediate=debug)
