...
rule merge_vcfs:
    ...
    shell:  
        """
        #bcftools concat -f {input.filelist} --threads {threads} -a --remove-duplicates -Ob --naive  --write-index > {output.bcf} 2> {log}
        bcftools concat -f {input.filelist} --threads {threads} -a --remove-duplicates -Ob --naive > {output.bcf} 2> {log}
...
rule log_runtime:
    ...
    run:
        with open(output[0], "w") as f:
            ...
            _ = f.write("\tbcftools concat -f vcf.list -a --remove-duplicates --naive\t")
