...
using("flexdashboard","magrittr","tidyr","DT","highcharter","RColorBrewer","scales")
...
#library(scales)
#dataL <- readLines("~/variants.raw.stats")
#bcf <- "filename.bcf"
...
```{r Individual plot, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%"}
...
highchart() |>
  ...
  hc_colors(RColorBrewer::brewer.pal(7, "Dark2")) |>
  ...
  hc_exporting(enabled = T, filename = "bcftools.stats",
...
if(do_qual){
  hchart(mergebins(qual[,-3]), "areaspline", hcaes(x = QualityScore, y = SNPs), color = "#8fbbbc", name = "count", marker = list(enabled = FALSE)) |>
    hc_title(text = "SNP Genotype Quality") |>
    hc_subtitle(text = "Shown in bins of width 10") |>
    hc_xAxis(title = list(text = "quality score")) |>
    hc_yAxis(title = list(text = "count")) |>
...
if(do_qual){
  hchart(mergebins(qual[,-2]), "areaspline", hcaes(x = QualityScore, y = Indels), name = "count", color = "#9393d2", marker = list(enabled = FALSE)) |>
...
if(sum(.dpL) == 0){
  ...
  hchart(dp, "areaspline", hcaes(x = Bin, y = NumberSites), name = "count", color = "#F28500", marker = list(enabled = FALSE)) |>
    hc_title(text = "Depth per Site") |>
    hc_xAxis(title = list(text = "bin"), type = "logarithmic") |>
    hc_yAxis(title = list(text = "number of sites")) |>
    hc_tooltip(crosshairs = TRUE,
      formatter = JS("function () {return '<b>bin ' + this.x + '</b><br>Count: <b>' + this.y + '</b>';}") 
    ) |>
    hc_exporting(enabled = T, filename = "indel.quality",
      buttons = list(contextButton = list(menuItems = c("viewFullscreen", "separator", "downloadPNG", "downloadJPEG", "downloadPDF", "downloadSVG")))
  )
...
if(sum(.iddL) == 0){
  ...
  hchart(idd, "areaspline", hcaes(x = Length, y = Count, group = Type, marker = list(enabled = FALSE))) |>
    ...
    hc_xAxis(title = list(text = "indel length"),
