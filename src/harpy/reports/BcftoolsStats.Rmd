...
hchart(tidy_psc, "scatter", hcaes(x = Count, y = Sample, group = Metric)) |>
...
if(do_qual){
  hchart(mergebins(qual[,-3]), "column", hcaes(x = QualityScore, y = SNPs), color = "#8fbbbc") |>
...
if(do_qual){
  hchart(mergebins(qual[,-2]), "column", hcaes(x = QualityScore, y = Indels), color = "#9393d2") |>
...
if(sum(.dpL) == 0){
  ...
  hchart(dp, "column", hcaes(x = Bin, y = NumberSites), color = "#F28500") |>
    hc_title(text = "Depth per Site") |>
    hc_xAxis(title = list(text = "bin")) |>
...
if(sum(.iddL) == 0){
  ...
  hchart(idd, "spline", hcaes(x = Length, y = Count, group = Type), color = "#F28500") |>
    hc_colors("#e6a037","#6daace") |>
