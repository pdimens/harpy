...
per unique molecule. These data are shown in 500 bp bins, _i.e._ the Y-value
...
```{r echo = FALSE, message = FALSE, warning = FALSE, out.width = '100%'}
p <- tb %>% filter(valid == "valid BX", length_inferred > 500) %>%
  arrange(length_inferred) %>% mutate(percent = 100/n() * 1:n()) %>%
  ggplot(d, aes(x = length_inferred, y = percent)) +
    geom_line(color = "#3b658d", linewidth = 2) +
    labs(title = "Inferred Haplotagged Molecule Lengths", subtitle = samplename) +
    xlab("Inferred Molecule Length (bp)") +
    ylab("% of Molecules (>500bp)") +
    theme_light() +
    scale_x_continuous(labels = scales::comma) +
...
- `mindist`: the minimum basepair distance between two alignments sharing a barcode (excluding read pairs, kind of a sanity check)
